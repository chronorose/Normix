...
static void panic_handler(int vector) {
    kernel_panic("unhandled interrupt %x\n", vector);
}
...
void ctx_print(ctx_t *ctx) {
    char *msg = "Kernel panic: unhandled interrupt %u, interrupted process context:\neax = %u, ecx = %u, edx = %u, ebx = %u, esp = %u, ebp = %u, esi = %u, edi = %u, ds = %u, es = %u, fs = %u, gs = %u, cs = %u, eip = %u\neflags (interrupted) = %u eflags (current) = %u, error code = %u";
